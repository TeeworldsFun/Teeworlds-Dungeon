...
void CMapGen::FillMap()
{
	...
	//if (str_comp(g_Config.m_SvGametype, "xxx") == 0)
		GenerateLevel();
	//else
	//	GeneratePVPLevel();
...
void CMapGen::GenerateLevel()
{
	...
	{
		for(int y = 6; y < h-1; y++)
			for(int x = 3; x < w-12; x++)
			{
				if (!pTiles->Get(x-6, y) && pTiles->Get(x, y) && pTiles->Get(x+6, y) && !pTiles->Get(x+12, y))
					pRoom->Fill(pTiles, 0, x, y, 4, 20);
				
				if (!pTiles->Get(x, y-6) && pTiles->Get(x, y) && pTiles->Get(x, y+6) && !pTiles->Get(x, y+12))
					pRoom->Fill(pTiles, 0, x, y, 4, 40);
			}
	}
